...
- Wiadomość zostaje oznaczona jako przeczytana po akcji użytkownika (np. "oznacz jako przeczytane") lub po wejściu na widok szczegółów powiadomienia.
...
- Zaimplementuj przykładowe powiadomienia w aplikacji:
  ...
  - Zmiana statusu zadania w historyjce (status done - medium, doing - low, otrzymuje właściciel historyjki)
- Zapamiętaj listę powiadomień 

#### Model powiadomienia
```js
type ISOString = string // lub inny typ
type Notification = {
  title: string,
  message: string,
  date: ISOString,
